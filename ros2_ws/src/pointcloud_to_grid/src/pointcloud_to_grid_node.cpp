...
public:
  PointCloudToGrid() : Node("pointcloud_to_grid_node"), count_(0)
  {
    ...
    this->declare_parameter<float>("cell_size", 0.0005);
...
private:
  ...
  lidar_callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr input_msg)
  {
    ...
    // count point on each cell
    std::vector<int> point_count(grid_map.cell_num_x * grid_map.cell_num_y, 0);

    // initialize grid vectors: -1 (unknown)
    for (auto &p : hpoints)
    {
      p = -1;
    }
      ...
      if (p.x > 0.01 || p.x < -0.01)
      {
        if (p.x > grid_map.bottomright_x && p.x < grid_map.topleft_x)
        {
          if (p.y > grid_map.bottomright_y && p.y < grid_map.topleft_y)
          {
            ...
            {
              point_count[cell.y * grid_map.cell_num_x + cell.x]++;
    ...
    for (pcl::PointXYZI p : out_cloud->points)
    {
      if (p.x > 0.01 || p.x < -0.01)
      {
        if (p.x > grid_map.bottomright_x && p.x < grid_map.topleft_x)
        {
          ...
          {
            PointXY cell = grid_map.getIndex(p.x, p.y);
            if (cell.x < grid_map.cell_num_x && cell.y < grid_map.cell_num_y && point_count[cell.y * grid_map.cell_num_x + cell.x] < 18)
            {
              ipoints[cell.y * grid_map.cell_num_x + cell.x] = -128;
              // hpoints[cell.y * grid_map.cell_num_x + cell.x] = -128;
            }
          }
        }
      }
