...
////    // Initialize the client to request parameters from RViz2
////    view_rviz_client = this->create_client<rcl_interfaces::srv::GetParameters>("/rviz/get_parameters", rmw_qos_profile_services_default,
////                                                                               client_cb_group_);

//    pcloud_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("vslam/pointcloud", 10);
//    pcloud_ref_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("vslam/refpointcloud", 10);
...
//    mark_pub_ = this->create_publisher<visualization_msgs::msg::Marker>("vslam/human_model_mesh", 10);
...
//    syncApproximate->registerCallback(&RGBDNode::GrabRGBD, this);

    // Timer to periodically check for camera information changes
//    view_rviz_timer_ = this->create_wall_timer(100ms, std::bind(&RGBDNode::rvizViewCallback, this), timer_cb_group_);
}
...
RGBDNode::RGBDNode(): Node("PhotoSLAM_ROS2")
{
    ...
    syncApproximate = std::make_shared<message_filters::Synchronizer<approximate_sync3_policy> >(approximate_sync3_policy(10), *rgb_sub, *depth_sub, *seg_sub);
...
void RGBDNode::initializedVSLAM()
{
    ...
    if (outputDir.back() != '/')
        outputDir += "/";
//    std::filesystem::path output_dir(outputDir);
...
void RGBDNode::GrabRGBD(const sensor_msgs::msg::Image::SharedPtr msgRGB, const sensor_msgs::msg::Image::SharedPtr msgD)
{
//    auto start = GET_MOMENT;
    ...
    if (cv_ptrRGB->image.empty() || cv_ptrRGB->image.empty()) {
        return;
    }
...
//    cv::waitKey(1);
    ...
    // Sophus::SE3f camera_pose_SE3 = pSLAM->TrackRGBD(im, depth, Utility::StampToSec(msgRGB->header.stamp),
...
//    auto stop = GET_MOMENT;
//    auto duration = GET_DURATION(stop, start);
    //    std::cout << "Time processing: " << duration/1000 << " ms" << std::endl;
//    this->publishCurrentDensePointCloud(depth, msgRGB->header.stamp);
    segment_publish(im, depth);
}

void RGBDNode::GrabRGBDS(const sensor_msgs::msg::Image::SharedPtr msgRGB, const sensor_msgs::msg::Image::SharedPtr msgD, const sensor_msgs::msg::Image::SharedPtr msgS)
{
    // Copy the ros rgb image message to cv::Mat.
    try
    {
    ...
    // Copy the ros segment image message to cv::Mat.
    try
    {
        cv_ptrS = cv_bridge::toCvShare(msgS);
    }
    catch (cv_bridge::Exception& e)
    {
        RCLCPP_ERROR(this->get_logger(), "cv_bridge exception: %s", e.what());
    ...
    cv::Mat im = cv::Mat(cv::Size(width_img, height_img), CV_8UC3, (void *)(cv_ptrRGB->image.ptr(0)), cv::Mat::AUTO_STEP);
    cv::Mat depth = cv::Mat(cv::Size(width_img, height_img), CV_16U, (void *)(cv_ptrD->image.ptr(0)), cv::Mat::AUTO_STEP);
    cv::Mat segment = cv::Mat(cv::Size(width_img, height_img), CV_8UC1, (void *)(cv_ptrS->image.ptr(0)), cv::Mat::AUTO_STEP);
...
//    std::vector<std::vector<float>> v_ref_pcloud;
...
//    RCLCPP_INFO(this->get_logger(), "Published %zu reference point clouds", ref_num_points);
#endif
}
...
//        RCLCPP_INFO(this->get_logger(), "Published current trajectory with %zu poses", path_msg.poses.size());
}
...
{
//    RCLCPP_INFO(this->get_logger(), "rvizViewCallback start");
...
//    RCLCPP_INFO(this->get_logger(), "rvizViewCallback wait");

//    // Wait for the result
//    if (rclcpp::spin_until_future_complete(this->get_node_base_interface(), result_future) != rclcpp::FutureReturnCode::SUCCESS)
//    {
//        RCLCPP_ERROR(this->get_logger(), "Failed to call service /rviz/get_parameters");
//        return;
//    }

//    RCLCPP_INFO(this->get_logger(), "rvizViewCallback Process");
}
...
void RGBDNode::rvizResponseHandler(rclcpp::Client<rcl_interfaces::srv::GetParameters>::SharedFuture future)
{
    ...
    if (current_camera_info != last_camera_info_)
    {
        ...
        // Publish the updated camera information
//        auto message = std_msgs::msg::String();
//        message.data = current_camera_info;
...
//    std::cout << human_height_ << std::endl;
    double head_height = 0.2 * human_height_;
    double body_height = 0.3 * human_height_;
    double leg_height = 0.5 * human_height_;
    double arm_length = 0.4 * human_height_;
...
//    // Transform the point from the camera frame to the local map frame using Twc
   Eigen::Vector4f world_human_position = Tcw * c_human_position;
    ...
    // Eigen::Vector4f world_human_position = c_human_position;
...
void RGBDNode::publish_human(std::vector<Human> humans_, Eigen::Matrix4f Tcw)
{
    ...
    {
//        RCLCPP_INFO(this->get_logger(), "Published a human");
        publish_human_marker_circle(human.height, human.position, marker_id, Tcw);
        marker_id += 1;
        // marker_id += 6;  // Increment by 6 because each human has 6 parts (head, body, 2 arms, 2 legs)
    }
}

void RGBDNode::segment_publish(cv::Mat rgbImg, cv::Mat depthImg)
{
    ...
    segmentModel_.infer(std::vector<cv::Mat>{rgbImg}, segmented);
...
//        humans.emplace_back(Human{real_height / 1000.0f, {point_world.x(), point_world.y(), point_world.z()}});
