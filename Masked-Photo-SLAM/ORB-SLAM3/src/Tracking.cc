...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &mask, const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::RGBD)
        mCurrentFrame = Frame(mask, mImGray,imDepth,mImRGB,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
    else if(mSensor == System::IMU_RGBD)
        mCurrentFrame = Frame(mask,mImGray,imDepth,mImRGB,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
