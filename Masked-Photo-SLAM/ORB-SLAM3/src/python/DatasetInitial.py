...
model = YOLO("/home/ducpa/Project/slam3d/models/yolov8x-seg.pt")
...
if __name__ == "__main__":
    ...
    source = os.path.join(args.data, 'rgb')
    maskpath = os.path.join(args.data, 'mask')
    removedpath = os.path.join(args.data, 'removed')
    
    
    print(f"Resulting masks will be saved to {maskpath}/, and frames with dynamic objects removed will be saved to {removedpath} \n")
    ...
    if not os.path.exists(removedpath):
        os.makedirs(removedpath)
    generate_mask(source,maskpath)
    remove_dyna(source,maskpath,removedpath)
